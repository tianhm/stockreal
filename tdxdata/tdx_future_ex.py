...
starttime = datetime.strptime('2016-01-01 21:45:00', '%Y-%m-%d %H:%M:%S')
print(starttime)
result, bars = api_01.get_bars('RB99', period='15min', start_dt=starttime, end_dt= datetime.now(), return_bar=False)

if result:
    with open('rb99_15.csv', 'a') as f:
        for j in range(len(bars)):
            bartmp = bars[j]
            bartmp['datetime'] = str(bartmp['datetime'])
            jsObj = json.dumps(bartmp)
            bartmp = json.dumps(bars[j])
            f.write(bartmp)
            f.write('\n')
...
# api_01.get_bars('RB99', period='15min', start_dt=stdt, callback=t1.display_bar, return_bar=True, bar_freq=1)
# 获取bar，只返回 list[dict]
